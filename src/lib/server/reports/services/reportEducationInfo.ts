import type { viewRow } from "$lib/types/manager.js";
...
// Map for Excel cell positions
const insertTypeAllDisabilitiesCellMap: Record<string, string> = {
...
const insertTypeDownSyndromeCellMap: Record<string, string> = {
...
export class ReportGeneratorEducationInfo extends ReportGenerator{
  ...
  static async insertAccessToEducValues(worksheet: ExcelJS.Worksheet){
    ...
    const { data: data1, error: err1 } = res1;
    
    const { data: data2, error: err2 } = res2;
    ...
    const updateCells = async (data: viewRow<'education_access_summary'>[] | viewRow<'education_type_summary'>[], map: Record<string, string>,) => {
      ...
      return Promise.all(data.flatMap(async (row)=>{
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const oddRows = [70, 72, 74, 76, 78].map(r => `${col}${r}`).join(',');
        const evenRows = [71, 73, 75, 77, 79].map(r => `${col}${r}`).join(',');
    ...
    await Promise.all([
      updateCells(allData, inserAccessAllCellMap),
      updateCells(downSyndromeData, insertAccessDownsyndromeCellMap)
    ])
  ...
  static async insertTypeOfEducValues(worksheet: ExcelJS.Worksheet) {
    ...
    const updateCells = async (data: viewRow<'education_type_summary'>[] | viewRow<'education_access_summary'>[], map: Record<string, string>) => {
      ...
      return Promise.all(data.flatMap(async (row)=>{
        ...
        await worksheet.getRow(Number(worksheet.getCell(cell).row)).commit()
      }))
    };
    ...
    const addFormulas = () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        //use rows in the MERGED sheet rather than the template.
        const oddRows = [87, 89, 91, 93, 95].map(r => `${col}${r}`).join(',');
        const evenRows = [88, 90, 92, 94, 96].map(r => `${col}${r}`).join(',');
    ...
    await Promise.all([
      updateCells(allData, insertTypeAllDisabilitiesCellMap),
      updateCells(downSyndromeData, insertTypeDownSyndromeCellMap)
    ])
  ...
  static async generateData(workbook: ExcelJS.Workbook, sheet: Worksheet) {
    const [res1, res2] = await Promise.all([
      this.insertAccessToEducValues(sheet),
      this.insertTypeOfEducValues(sheet)
    ]);
