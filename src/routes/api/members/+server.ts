...
export const PUT: RequestHandler = async({request}) => {
  ...
  // Check for member info updates
  const memberUpdates: any = {}
  if (body.first_name !== undefined) {
    memberUpdates.first_name = body.first_name
    hasUpdates = true
  }
  if (body.last_name !== undefined) {
    memberUpdates.last_name = body.last_name
    hasUpdates = true
  }
  if (body.middle_name !== undefined) {
    memberUpdates.middle_name = body.middle_name
    hasUpdates = true
  }
  if (body.birthday !== undefined) {
    memberUpdates.birthday = body.birthday
    hasUpdates = true
  }
  if (body.sex !== undefined) {
    ...
    memberUpdates.sex = body.sex
    hasUpdates = true
  }

  if (Object.keys(memberUpdates).length > 0) {
    const updated = await membersModel.instance.updateMemberInfo(body.id, memberUpdates)
    if (!updated) {
      throw error(500, 'Failed to update member info')
    }
  }
